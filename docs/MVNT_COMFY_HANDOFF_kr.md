...
- `dance_video`: 서버가 렌더한 MP4를 Comfy native `VIDEO` 객체로 감싼 출력입니다.

내부적으로는 MP4를 로컬 output 폴더에 받은 뒤 `InputImpl.VideoFromFile(...)`로 감싸서 반환합니다. 그래서 Comfy 기본 `SaveVideo`처럼 `VIDEO` 입력을 요구하는 노드에 바로 연결할 수 있습니다.
...
- 서버 MP4 다운로드
- `dance_video`를 Comfy native `VIDEO` 출력으로 전달
...
- 오래된 workflow나 문서에는 `dance_video`가 path string이라고 적혀 있을 수 있습니다. 현재 코드는 native `VIDEO` 객체로 반환합니다.
