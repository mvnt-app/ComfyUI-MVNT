...
이번 작업은 ComfyUI 안에서 MVNT 춤 생성 흐름을 팀원이 이어받을 수 있게 정리한 작업입니다. 오디오를 넣으면 MVNT가 먼저 춤 GLB를 만들고, 필요하면 별도 노드로 서버 MP4를 요청합니다. 빠른 검토는 GLB 3D preview로 하고, 느린 MP4 렌더는 필요할 때만 연결하는 것이 핵심 기준입니다.
...
첫 번째, ComfyUI 쪽에서는 `MVNT Generate Dance`를 메인 생성 노드로 봅니다. 오디오를 넣고, 스타일을 고르고, 필요하면 캐릭터 GLB를 연결하면 이 노드가 MVNT 서버에 요청해서 preview 가능한 `dance_3d` GLB를 만듭니다.
...
네 번째, MP4는 `MVNT Render Dance Video`로 분리했습니다. 이 노드는 `dance_3d`를 연결했을 때만 서버 Blender 렌더를 요청합니다. 그래서 3D만 확인하려는 workflow가 MP4 렌더 대기 시간에 잡히지 않습니다.
...
`MVNT Generate Dance`는 의도적으로 `VIDEO`를 반환하지 않습니다. GLB 생성과 서버 MP4 렌더를 분리해서, 빠른 3D preview 경로가 Blender 렌더를 기다리지 않게 하기 위해서입니다.

### MVNT Render Dance Video

선택형 MP4 노드입니다. `MVNT Generate Dance`의 `dance_3d`를 연결하면 파일명에서 job id를 추론하고, 서버 render endpoint에서 MP4를 받아 Comfy native `VIDEO` 객체로 반환합니다.

현재 노드는 width/height/fps 위젯을 노출하지 않습니다. 내부 요청은 `670x400 @ 30fps`로 고정되어 있고, 서버 Blender 카메라가 전신 reference를 잘 담도록 조정되어 있습니다. `pretty`는 일반 preview용, `kling`은 Kling motion transfer reference용 profile입니다.
...
현재 Comfy 코드는 새 `/v1/generations` 형태를 먼저 시도하지만, 실사용 fallback은 `https://api.mvnt.studio`의 legacy/live endpoint에 많이 의존합니다. nginx 기준으로 generation 계열은 LDA/render lane으로 나뉘어 들어갑니다.

- `/v1/generations`: 새 API 형태 우선 시도
- `/generate-motion-lda`: legacy 춤 생성 요청
- `/job-lda`: 생성 상태 확인
- `/download-bvh-lda`, `/download-glb-lda`, `/export-glb-bvh-lda`: motion/GLB output 확보
- `/retarget-tripo-glb`: Tripo 캐릭터에 춤 입히기
- `/render-mp4-lda`: 서버 Blender MP4 렌더

관련 호출 흐름은 `docs/MVNT_API_SERVER_FLOW_kr.md`를 먼저 보면 됩니다.

## 7. 파일별 담당 구역

### ComfyUI-MVNT repo

- `nodes.py`: Comfy 노드 입력/출력과 실행 로직입니다. `MVNT Audio Segment`, `MVNT Image to T-Pose`, `MVNT Generate Dance`, `MVNT Render Dance Video`, `MVNT Preview Dance 3D`가 여기서 정의됩니다.
- `mvnt_client.py`: MVNT API, legacy API fallback, output download, retarget, Tripo 호출 래퍼입니다.
- `__init__.py`: ComfyUI가 MVNT 노드를 등록하는 입구입니다. 현재 메뉴에는 핵심 4개 노드만 등록합니다.
...
키는 기본적으로 MVNT generation 키 하나만 workflow 사용자가 다룹니다.

- MVNT generation: `MVNT_API_KEY` 또는 `api_key` 입력입니다. 현재 `mvnt_*`와 legacy `mk_*` prefix를 받습니다.
- T-pose image regeneration: ComfyUI가 MVNT image preprocessing endpoint를 호출합니다. provider API key는 서버에만 있고 workflow JSON이나 노드 입력에 넣지 않습니다.
...
- `MVNT Render Dance Video`를 연결한 경우에만 서버 MP4 다운로드
- 선택형 `dance_video`를 Comfy native `VIDEO` 출력으로 전달
- MVNT T-pose image preprocessing 요청
- character GLB가 연결된 경우 retarget endpoint 호출
- Comfy native 3D preview와 audio payload 연결
- Preview 3D payload 저장. 다른 workflow로 갔다 돌아와도 마지막 GLB를 다시 mount할 수 있게 보존
- 팀 리뷰용 cleaned full workflow 제공

## 10. 아직 조심할 것

- full mS/Kling graph는 외부 custom node가 있어야 그대로 열립니다. 없으면 missing node가 뜹니다.
- 오래된 workflow나 문서에는 `MVNT Generate Dance`가 바로 `dance_video`를 반환한다고 적혀 있을 수 있습니다. 현재 기준은 별도 `MVNT Render Dance Video` 노드입니다.
